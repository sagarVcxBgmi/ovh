...
total_attack_count = 0  # New global counter for total attacks
...
def handle_bgmi(message):
    ...
    try:
        proc = subprocess.Popen(
            [
                "./240", target, str(port), str(duration), "900"
            ],
...
- /feedback_count - Display the total Hit and Not Hit feedback counts along with total attacks.
...
- For assistance, contact the owner @wtf_vai⚡️.
    """
    try:
...
def feedback_count_command(message):
    global total_hit_count, total_not_hit_count, total_attack_count
